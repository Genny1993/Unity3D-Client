...
public class SettingsWindow : MonoBehaviour
{
    ...
    private async void Start()
    {
        ...
        var formData = new List<KeyValuePair<string, string>>
        {
            ...
            new KeyValuePair<string, string>("pack[method]", "getName"),
        ...
        try
        {
            ...
            nameField.text = result["info"][0]["name"]?.ToString() ?? "";
        }
        catch (Exception)
        {
        }
    }
    private async void OnNameClicked()
    {
        ...
        {
            new KeyValuePair<string, string>("pack[service]", "account"),
            new KeyValuePair<string, string>("pack[method]", "changeName"),
            new KeyValuePair<string, string>("pack[access_key]", Settings.AuthKey),
            new KeyValuePair<string, string>("pack[info][name]", nameField.text.Trim())
        };
        ...
        try
        {
            ...
            Newtonsoft.Json.Linq.JObject result = await Sender.SendAndGet(formData);
            MessageBox.Show("Информация", "Вы сменили отображаемое имя");
        }
        catch (Exception)
        {
        }
    ...
    private async void OnPasswordClicked()
    {
        AudioManager.PlayOneShot(buttonClick, clickVolume);
        ...
        {
            new KeyValuePair<string, string>("pack[service]", "account"),
            new KeyValuePair<string, string>("pack[method]", "changePassword"),
            new KeyValuePair<string, string>("pack[access_key]", Settings.AuthKey),
            new KeyValuePair<string, string>("pack[info][password]", passwordField.text.Trim())
        };
        ...
        try
        {
            ...
            Newtonsoft.Json.Linq.JObject result = await Sender.SendAndGet(formData);
            passwordField.text = "";
            MessageBox.Show("Информация", "Вы сменили пароль");
        }
        catch (Exception)
        {
        }
